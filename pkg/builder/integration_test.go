...
`
	...
	// link 应该有两个输入
	assert.Len(t, linkEdge.Inputs, 2)
}
...
`
	...
	assert.Equal(t, "foo.h", edge.Inputs[1].Path)
}
...
`
	...
	// dyndep 文件节点应该被标记
	require.NotNil(t, edge.DyndepFile)
	assert.True(t, edge.DyndepFile.DyndepPending)
}
...
`
	...
	assert.Equal(t, "check.py", edge.Validations[0].Path)
}
...
`
	...
	assert.Equal(t, "parser.cc", edge.Outputs[0].Path)
	assert.Equal(t, "parser.h", edge.Outputs[1].Path)
}
