...
func (q *EdgePriorityQueue) Less(i, j int) bool {
	// 按 critical path weight 降序，再按 ID 升序
	if q.edges[i].CriticalPathWeight != q.edges[j].CriticalPathWeight {
		return q.edges[i].CriticalPathWeight > q.edges[j].CriticalPathWeight
	}
	return q.edges[i].ID < q.edges[j].ID
}
