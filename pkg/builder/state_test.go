...
func TestNewState(t *testing.T) {
	...
	assert.Contains(t, state.Pools, "console")
...
// TestState_GetNodeByID 测试通过 ID 获取节点
...
func TestState_AddEdge(t *testing.T) {
	...
	assert.Equal(t, uint64(0), edge.ID)
	assert.Equal(t, DefaultPool, edge.Pool)
	assert.Equal(t, state.Bindings, edge.Env)

	// 添加第二条边，ID 应该递增
	edge2 := state.AddEdge(rule)
	assert.Equal(t, uint64(1), edge2.ID)
}
...
func TestState_AddIn(t *testing.T) {
	...
	assert.Equal(t, "input.c", edge.Inputs[0].Path)
...
func TestState_Reset(t *testing.T) {
	...
	assert.False(t, edge.DepsLoaded)
...
func TestEdge_UseConsole(t *testing.T) {
	...
	assert.True(t, consoleEdge.UseConsole())

	// 默认池
	defaultEdge := &Edge{Pool: DefaultPool}
	assert.False(t, defaultEdge.UseConsole())

	// 无池
	noPoolEdge := &Edge{}
	assert.False(t, noPoolEdge.UseConsole())
}
...
func TestEdge_AllInputsReady(t *testing.T) {
	...
	// 创建输入节点和边
	in1 := &Node{Path: "in1"}
	in1Edge := &Edge{Rule: rule, OutputsReady: true}
	in1.InEdge = in1Edge
...
func TestEdge_MaybePhonyCycleDiagnostic(t *testing.T) {
	...
	// 非 phony 规则
	ccRule := &Rule{Name: "cc"}
	edge2 := &Edge{Rule: ccRule, Outputs: []*Node{out}}
	assert.False(t, edge2.MaybePhonyCycleDiagnostic())

	// 多个输出
	edge3 := &Edge{
		Rule:    phonyRule,
		Outputs: []*Node{{Path: "out1"}, {Path: "out2"}},
	}
