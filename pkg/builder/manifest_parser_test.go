...
`
	...
	assert.Equal(t, "foo.c", edge.Inputs[0].Path)
	require.Len(t, edge.Outputs, 1)
	assert.Equal(t, "foo.o", edge.Outputs[0].Path)
}
...
`
	...
	assert.Equal(t, "foo.o", edge.Inputs[0].Path)
	assert.Equal(t, "bar.o", edge.Inputs[1].Path)
	assert.Equal(t, "baz.o", edge.Inputs[2].Path)
}
...
`
	...
	assert.Equal(t, "out2", edge.Outputs[1].Path)
	assert.Equal(t, "out3", edge.Outputs[2].Path)
}
...
`
	...
	require.Len(t, edge.Inputs, 3)
	assert.Equal(t, "foo.c", edge.Inputs[0].Path)
	assert.Equal(t, "header.h", edge.Inputs[1].Path)
	assert.Equal(t, "config.h", edge.Inputs[2].Path)
	assert.Equal(t, 2, edge.ImplicitDeps)
}
...
`
	...
	assert.Equal(t, "foo.c", edge.Inputs[0].Path)
	assert.Equal(t, "stamp", edge.Inputs[1].Path)
...
`
	...
	assert.Equal(t, "foo.o", edge.Outputs[0].Path)
	assert.Equal(t, "foo.h", edge.Outputs[1].Path)
...
`
	...
	assert.Equal(t, "validate.py", edge.Validations[0].Path)
}
...
`
	...
	// 自引用应该被处理（警告模式下）
	edge := state.Edges[0]
	assert.Len(t, edge.Inputs, 0) // 自引用被移除
}
