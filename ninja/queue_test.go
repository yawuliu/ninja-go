...
// TestEdgePriorityQueue_Less_ByID 测试按 id_ 比较
func TestEdgePriorityQueue_Less_ByID(t *testing.T) {
	...
	// id_ 小的优先
	assert.True(t, queue.Less(1, 0)) // edge2 (id_=1) < edge1 (id_=2)
	assert.False(t, queue.Less(0, 1))
}
...
func TestEdgePriorityQueue_HeapInterface(t *testing.T) {
	...
	// 弹出应该按权重降序，同权重按 id_ 升序
	first := heap.Pop(queue).(*Edge)
	assert.Equal(t, int64(10), first.critical_path_weight_)
	assert.Equal(t, uint64(2), first.id_)

	second := heap.Pop(queue).(*Edge)
	assert.Equal(t, int64(10), second.critical_path_weight_)
	assert.Equal(t, uint64(4), second.id_)

	third := heap.Pop(queue).(*Edge)
	assert.Equal(t, int64(5), third.critical_path_weight_)

	fourth := heap.Pop(queue).(*Edge)
	assert.Equal(t, int64(3), fourth.critical_path_weight_)
}
...
func TestEdgePriorityQueue_PriorityOrder(t *testing.T) {
	...
	// 弹出验证顺序
	top := queue.Top()
	assert.Equal(t, int64(100), top.critical_path_weight_)
	assert.Equal(t, uint64(1), top.id_) // 同权重，id_ 小的优先
}
...
func TestEdgePriorityQueue_MultipleOperations(t *testing.T) {
	...
	for queue.Len() > 0 {
		...
		// 权重应该递减或相等
		assert.LessOrEqual(t, edge.critical_path_weight_, prevWeight)
		prevWeight = edge.critical_path_weight_
		count++
	}
